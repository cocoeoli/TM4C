...
#define i2c_write   I2CWriteRegMultiByte_MPU
#define i2c_read    I2CReadRegMultiByte_MPU
...
int mpu_init(void)
{
    ...
    /* Reset device. */
    data[0] = BIT_RESET;
    if (I2CWriteRegOneByte(I2C0_BASE, 0x68, 0x6B, 0x80))
        return -1;
    delay_ms(100);

    /* Wake up chip. */
    data[0] = 0x00;
    if (I2CWriteRegOneByte(I2C0_BASE, st.hw->addr, st.reg->pwr_mgmt_1, data[0]))
        return -1;

#if defined MPU6050
    /* Check product revision. */
    if (I2CReadRegMultiByte(I2C0_BASE, st.hw->addr, st.reg->accel_offs, data, 6))
        return -1;
    ...
    if (rev) {
        ...
        if (I2CReadRegOneByte(I2C0_BASE, st.hw->addr, st.reg->prod_id, data))
            return -1;
        rev = data[0] & 0x0F;
        if (!rev) {
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
    if (I2CWriteRegOneByte(I2C0_BASE, st.hw->addr, st.reg->gyro_cfg, data))
        return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 3))
        return 0;
    if (I2CWriteRegOneByte(I2C0_BASE, st.hw->addr, st.reg->accel_cfg, data))
        return -1;
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (st.chip_cfg.lpf == data)
        return 0;
    if (I2CWriteRegOneByte(I2C0_BASE, st.hw->addr, st.reg->lpf, data))
        return -1;
    ...
    unsigned char *more)
{
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        if (I2CReadRegOneByte(I2C0_BASE, st.hw->addr, st.reg->int_status, tmp))
            return -1;
    ...
    if( (length==1) && I2CReadRegOneByte(I2C0_BASE, st.hw->addr, st.reg->fifo_r_w, data))
        return -1;
    else if(i2c_read(st.hw->addr, st.reg->fifo_r_w, length, data))
        return -1;
    else
        ;
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (I2CReadRegOneByte(I2C0_BASE, st.hw->addr, st.reg->user_ctrl, &tmp))
            return -1;
        tmp &= ~BIT_AUX_IF_EN;
        if (I2CWriteRegOneByte(I2C0_BASE, st.hw->addr, st.reg->user_ctrl, tmp))
            return -1;
        delay_ms(3);
        tmp = BIT_BYPASS_EN;
        ...
        if (I2CWriteRegOneByte(I2C0_BASE, st.hw->addr, st.reg->int_pin_cfg, tmp))
            return -1;
    } else {
...
//            DMP_FEATURE_GYRO_CAL);
        res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT                 //更改设置dmp功能
         |DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
            DMP_FEATURE_GYRO_CAL );
        if(res)return 6;
//        res=dmp_set_fifo_rate(180);  //设置DMP输出速率(最大不超过200Hz)
//        if(res)return 7;
...
//    return 0;
//}
uint8_t mpu_dmp_get_data(float *yaw)
{
    float q0=1.0f,q1=0.0f,q2=0.0f,q3=0.0f;
        unsigned long sensor_timestamp;
        short gyro[3], accel[3], sensors;
